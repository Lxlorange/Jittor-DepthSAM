...
def get_attn_bias_and_cat(x_list, branges=None):
    """
    this will perform the index select, cat the Vars, and provide the attn_bias from cache
    """
    ...
    if branges is not None:
        cat_Vars = index_select_cat([x.flatten(1) for x in x_list], branges).view(1, -1, x_list[0].shape[-1])
    else:
        Vars_bs1 = tuple(x.reshape([1, -1, *x.shape[2:]]) for x in x_list)
        cat_Vars = jt.concat(Vars_bs1, dim=1)

    return attn_bias_cache[all_shapes], cat_Vars


def drop_add_residual_stochastic_depth_list(
    x_list: List[Var],
    residual_func: Callable[[Var, Any], Var],
    sample_drop_ratio: float = 0.0,
    scaling_vector=None,
) -> Var:
    ...
    # 2) get attention bias and index+concat the Vars
...
class NestedVarBlock(Block):
    def forward_nested(self, x_list: List[Var]) -> List[Var]:
        """
        x_list contains a list of Vars to nest together and run
        """
        assert isinstance(self.attn, MemEffAttention)

        if self.training and self.sample_drop_ratio > 0.0:

            def attn_residual_func(x: Var, attn_bias=None) -> Var:
                return self.attn(self.norm1(x), attn_bias=attn_bias)

            def ffn_residual_func(x: Var, attn_bias=None) -> Var:
