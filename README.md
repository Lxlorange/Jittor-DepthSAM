...
如果运行 `Attention_SD` 中的 FFT 路径，必须启用 CUDA。当前环境曾遇到 Jittor cuFFT JIT 编译错误，错误原因是 Jittor 生成的 cuFFT op 源码缺少 `#include <array>`。本地修复方式是在 Jittor 安装目录的如下文件中补充头文件：
...
| 实验 | 配置 | 结果 |
| --- | --- | --- |
| 原 Jittor full train | batchsize=1 | 约 `648 ms/iter` |
| Jittor 方案 A full train | batchsize=2 | 约 `526 ms/iter`，约 `3.80 FPS` |
...
- 当前 `test.py` 默认只测试 CAMO，完整四数据集测试需要扩展 `test_datasets`。
- 当前 README 记录了已有训练 loss 趋势、性能 profiler 和历史可视化结果，但完整的四数据集 MAE 表还需要在完成 Jittor 长训后补充。
...
8. 资源允许时扩展到 CAMO / CHAMELEON / COD10K / NC4K 四个测试集，并在 README 中补充最终 MAE 表。
